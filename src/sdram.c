...
#define logf(fmt, ...) SendString(fmt)
//#define logf(fmt, ...)
#define panic(a, ...) logf(a)


dword g_RAMSize = RAM_SIZE_UNKNOWN;

static const char* lpddr2_manufacturer_name(dword mr) {
	switch (mr) {
	case 1:
		return "Samsung";
	case 2:
...
static unsigned lpddr2_size(dword mr) {
	switch (mr) {
	case 0x58:
		return RAM_SIZE_1GB;
	case 0x18:
...
static void selftest_at(dword addr) {
	volatile dword* ram = (volatile dword*)addr;
